...
with st.sidebar:
    ...
    mode = st.radio(
        ...
        ["safe", "fast"],
        index=0,
        help="Safe mode uses fewer workers and saves more often."
    )

    enrich_emails = st.checkbox(
        "Email enrichment",
        value=False,
        help="Off by default for stability on large jobs."
    )
...
refresh_clicked = c3.button("Refresh status", use_container_width=True)
